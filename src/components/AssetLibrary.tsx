...
export const AssetLibrary: React.FC<AssetLibraryProps> = ({ isOpen, onClose }) => {
  ...
  const handlePasteSave = () => {
    ...
    addAsset(pasteName || 'Untitled Paste', pasteContent);
    setPasteContent('');
    setPasteName('');
    setIsPasting(false);
  };
        ...
        {!isPasting ? (
          <div className="p-4 border-b border-gray-100 space-y-4">
              ...
              <div className="flex gap-2">
                  <button 
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-blue-50 border border-blue-100 text-blue-700 rounded-lg text-sm font-medium hover:bg-blue-100 transition-colors"
                  >
          ...
          <div className="p-4 border-b border-gray-100 space-y-3 bg-gray-50/50">
            <div className="flex items-center justify-between">
               <h3 className="text-sm font-semibold text-gray-700">Paste Markdown</h3>
               <button onClick={() => setIsPasting(false)} className="text-xs text-gray-500 hover:text-gray-700">Cancel</button>
            </div>
            <input
              type="text"
              ...
              className="w-full px-3 py-2 bg-white border border-gray-200 rounded-md text-sm focus:outline-none focus:border-blue-500"
            />
            <textarea
              placeholder="# Paste your markdown content here..."
              value={pasteContent}
              onChange={(e) => setPasteContent(e.target.value)}
              className="w-full h-32 px-3 py-2 bg-white border border-gray-200 rounded-md text-sm font-mono resize-none focus:outline-none focus:border-blue-500"
            />
            <button
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {filteredAssets.length === 0 ? (
                <div className="text-center py-10 text-gray-500">
                    ...
                    <p className="text-xs text-gray-400 mt-1">Import files or paste content to get started.</p>
                </div>
            ) : (
